...
class MatplotlibWidget(QtGui.QWidget):
    ...
    def onmove(self, event):
        ...
        try:
            ...
            self.emit(QtCore.SIGNAL('MouseMove(int,QString)'),x,y)
        except:
            pass

    def onleave(self, event):
        self.emit(QtCore.SIGNAL('MouseLeave'))
...
class MWindow(QtGui.QMainWindow,Ui_MainWindow):
    def __init__(self):
        ...
        QtCore.QObject.connect(self.action_Exit, QtCore.SIGNAL("triggered()"), self, QtCore.SLOT("close()"))#close event
        #activate function
        self.activate()
        self.loadPorts()
    ...
    def activate(self):
        ...
        #
        self.matplotlibWidget.axes.clear()#clear axes
        #
    ...
    def viewData(self,num,mode,val):
        ...
        if len(self.mass_ac_voltage) != 0:
            self.matplotlibWidget.axes.plot(self.mass_ac_voltage_x, self.mass_ac_voltage,'r.-')#,label='Voltage,V')
            self.matplotlibWidget.axes.set_ylabel('V',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
            range_ = self.comboBox_4.currentIndex()
            if range_ == 0:
                self.matplotlibWidget.axes.set_ylim([-1,1])
            elif range_ == 1:
                self.matplotlibWidget.axes.set_ylim([-5,5])
            elif range_ == 2:
                self.matplotlibWidget.axes.set_ylim([-12,12])
            else:
                pass
        elif len(self.mass_dc_voltage) != 0:
            self.matplotlibWidget.axes.plot(self.mass_dc_voltage_x, self.mass_dc_voltage,'r.-')#,label='Voltage,V')
            self.matplotlibWidget.axes.set_ylabel('V',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
            range_ = self.comboBox_4.currentIndex()
            if range_ == 0:
            ...
            self.matplotlibWidget.axes.plot(self.mass_ac_current_x, self.mass_ac_current,'g.-')#,label='Current,A')
            self.matplotlibWidget.axes.set_ylabel('A',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
            range_ = self.comboBox_4.currentIndex()
            if range_ == 0:
                self.matplotlibWidget.axes.set_ylim([-200,200])
            elif range_ == 1:
                self.matplotlibWidget.axes.set_ylim([-10,10])
            else:
                pass
        elif len(self.mass_dc_current) != 0:
            self.matplotlibWidget.axes.plot(self.mass_dc_current_x, self.mass_dc_current,'g.-')#,label='Current,A')
            self.matplotlibWidget.axes.set_ylabel('A',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
            range_ = self.comboBox_4.currentIndex()
            if range_ == 0:
            ...
            self.matplotlibWidget.axes.plot(self.mass_resistance_x, self.mass_resistance,'b.-')#,label='Resistance, Om')
            self.matplotlibWidget.axes.set_ylabel('Ohm',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
            range_ = self.comboBox_4.currentIndex()
            if range_ == 0:
                self.matplotlibWidget.axes.set_ylim([0,1000])
            elif range_ == 1:
                self.matplotlibWidget.axes.set_ylim([0,1000000])
            else:
                pass
        elif len(self.mass_test_continuity) != 0:
            self.matplotlibWidget.axes.plot(self.mass_test_continuity_x, self.mass_test_continuity,'m.-')#,label='Resistance, Om')
            self.matplotlibWidget.axes.set_ylabel('Test',rotation=0)
            self.matplotlibWidget.axes.set_xlabel('time unit')
        elif len(self.mass_logic_level) != 0:
            self.matplotlibWidget.axes.plot(self.mass_logic_level_x, self.mass_logic_level,'k.-')#,label='Resistance, Om')
            self.matplotlibWidget.axes.set_ylabel('HIGH,LOW',rotation=90)
            self.matplotlibWidget.axes.set_xlabel('time unit')
        else:
            pass
        self.matplotlibWidget.canvas.draw()#redraw
                ...
                if self.mode == 0:
                    ...
                    if text:
                        ...
                        if len(tokens) == 3:
                            ...
                            self.emit(QtCore.SIGNAL('OutputData(QString,QString,QString)'),str(number),str(typ),str(value))#signal for update
